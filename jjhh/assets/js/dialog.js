(function (global, $, pandora, undefined) {
    ...
    function Factory(config, ok, cancel) {
        ...
        return Dialog.list[config.id] = universe 
                ? universe._init(config) 
                : new Dialog(config);
    }
    ...
    Dialog.prototype = {
        ...
        _init: function (config) {
            ...
            this.updateTemplateBgBox(config.templateBgBox);
        ...
        updateTemplateBgBox: function(bgBox){
            var len = bgBox.length;
            this.wrap.find('.dialog-xbgbox').remove();
            if(!len) return;
            var tempHtml = '<div class="dialog-xbgbox">';
            for(var i=0;i<len;i++){
                tempHtml += '<div class="'+ bgBox[i] +'"></div>';
            }
            tempHtml += '</div>';
            this.wrap.append(tempHtml);
        },
        /**
         * 获取 Dialog 模板
         * REVIEW
        ...
        _mask: function (config) {
            ...
            //fixed: 弹窗出现时调用msg自动隐藏，关闭此弹窗后再显示，则丢了遮罩层
            else{
                $("[data-mask=overlay]").show();
            }
